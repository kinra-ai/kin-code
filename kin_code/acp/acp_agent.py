...
The implementation handles three different launch modes:
    1. Dev mode: `uv run kin-acp` from project root
    2. UV tool install: `kin-acp` with UV path resolution
...
from kin_code.acp.utils import (
    TOOL_OPTIONS,
    ToolOption,
    acp_to_agent_mode,
...
from kin_code.core.agent import Agent as KinAgent
from kin_code.core.autocompletion.path_prompt_adapter import render_path_prompt
from kin_code.core.config import KinConfig, MissingAPIKeyError, load_api_keys_from_env
...
class KinAcpAgent(AcpAgent):
    ...
    async def initialize(self, params: InitializeRequest) -> InitializeResponse:
        ...
        #  - dev mode: `uv run kin-acp`, ran from the project root
        #  - uv tool install: `kin-acp`, similar to dev mode, but uv takes care of path resolution
        #  - bundled binary: `./kin-acp` from binary location
        ...
        # So sys.executable is already /path/to/binary/kin-acp.
    ...
    async def newSession(self, params: NewSessionRequest) -> NewSessionResponse:
        ...
        try:
            config = KinConfig.load(
                workdir=cwd,
                tool_paths=[str(KIN_ROOT / "acp" / "tools" / "builtins")],
        ...
        agent = KinAgent(config=config, mode=AgentMode.DEFAULT, enable_streaming=True)
    ...
    async def setSessionModel(
        ...
        KinConfig.save_updates({"active_model": params.modelId})
...
async def _run_acp_server() -> None:
    ...
    AgentSideConnection(lambda connection: KinAcpAgent(connection), writer, reader)
