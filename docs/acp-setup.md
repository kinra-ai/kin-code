...
For usage in Zed, we recommend using the [Kin Code Zed's extension](https://zed.dev/extensions/kin-code). Alternatively, you can set up a local install as follows:
...
```lua
{
  acp_providers = {
    ["kin-code"] = {
      command = "kin-acp",
      env = {
         KIN_API_KEY = os.getenv("KIN_API_KEY"), -- necessary if you setup Kin Code manually
      },
    }
  }
}
